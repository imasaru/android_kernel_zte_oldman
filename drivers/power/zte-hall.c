...
extern int offcharging_flag; //zhaoyun for offcharging

struct hall_pwrkey *hallpwrkey;
struct wake_lock hall_wake_lock;
#define HALL_WAKELOCK_TIMEOUT 14
static int hall_mesc = 120;
static int hall_count=0; //zhaoyun_20160322_pm add for detecting hall when manufacture 
...
extern void hall_state_acquire_zte(bool hall_state);
#endif
...
module_param_named(hall_interrupt_count, hall_count, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
//add by pingzhenghai for tp suspend
#ifdef CONFIG_BOARD_OLDMAN
	hall_state_acquire_zte(open);
#endif

	//zte_zhaoyunz_pm_20160128, if offcharging report powerkey by hall
	if(offcharging_flag && (!hall_current_factory_mode)) {
		...
		input_sync(hallpwrkey->hall_pwr);
		pr_info("offcharging:hallstate=%d\n" , hallstate);
        }
...
//add by pingzhenghai for tp suspend
	...
	bool open;
	open = gpio_get_value(gpio_hall_en);////0:close;1:open
	printk("%s:hall interrupt, state=%s\n", __func__, open?"open":"close");
	hall_state_acquire_zte(open);
#endif
	...
	//zte_pm_20160324 count in PV
	hall_count +=1;

	if(hall_mesc&DEBUG_HALL_STATE) {
		pr_info("ZTE_PM_HALL gonna call update_hall_state_by_work in hall_detect_interrupt\n");
	}
	return IRQ_HANDLED;
}
...
static int  zte_hall_probe(struct platform_device *pdev) {
	...
	//zte_pm_zhaolq_20151214 change KEY_POWER to uevent
