...
extern int is_f3mem_range(unsigned long pfn, unsigned long size);
...
static int __init msm_tb_dev_init(void)
{
    ...
    if (!np) {
      pr_err("unable to find DT imem sdlog mem addr node\n");
    } else {
    ...
    if (!np) {
      pr_err("unable to find DT imem sdlog mem size node\n");
    } else {
