...
extern int socinfo_get_recovery_flag(void);
					...
					struct mdp_display_commit *cmt_data)
{
	...
	if (mdp3_session->first_commit) {
		...
		if (panel_info->mipi.post_init_delay)
			...
			#if defined CONFIG_BOARD_BAFFIN ||defined CONFIG_BOARD_LAVENDER 
			msleep(3000 / panel_info->mipi.frame_rate);
			#else
			msleep(1000 / panel_info->mipi.frame_rate);
			#endif
		pr_err("%s, wait to ensure frame is sent to panel\n", __func__);
	...
	if (!splash_done || mdp3_session->esd_recovery == true) {
		if(panel && panel->set_backlight)
			panel->set_backlight(panel, 96);
		pr_err("%s, set_backlight=bl_max\n", __func__);
		splash_done = true;
...
static void mdp3_ctrl_pan_display(struct msm_fb_data_type *mfd)
{
	...
	if (!splash_done || mdp3_session->esd_recovery == true) {
		...
		if(panel && panel->set_backlight){
			if(socinfo_get_recovery_flag())
			panel->set_backlight(panel, panel->panel_info.bl_max);
			}
		#else
		if(panel && panel->set_backlight&&(offcharging_flag!=1)){
			panel->set_backlight(panel, panel->panel_info.bl_max);
		pr_err("%s, set_backlight=bl_max\n", __func__);
			}
		#endif
