...
void mdss_convert_chip_reset(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	printk("tangzhengbo mdss_convert_chip_reset\n");

	gpio_direction_output((ctrl_pdata->convert_rst_gpio), enable);	
	msleep(5);

	gpio_direction_output((ctrl_pdata->convert_rst_gpio), 0);
	msleep(5);
...
void mdss_dsi_panel_3v_power(struct mdss_panel_data *pdata, int enable)
{
	...
	printk("tangzhengbo mdss_dsi_panel_3v_power\n");
...
/*
allow surfaceflinger system_data_file:file { create open read write};
allow surfaceflinger system_data_file:dir { write add_name };
*/
#define MAX_INITIAL_CODE_LEN 2048
static char initial_code_buf[MAX_INITIAL_CODE_LEN]={0};
static int initial_code_len;
static int first_time_on_flag = 1;


static void convert_one_byte_to_ascii(unsigned char value,char* buf)
{	
	unsigned char  val = value,i,tmp[2];
	tmp[1]=val&0x0f;
	tmp[0]=val>>4;
	for (i=0;i<2;i++)
	{
		...
		//printk("LCD new convert =%x\n",*(buf+i));
	}
}

static int convert_lcd_initial_code_to_ascii(struct dsi_panel_cmds *pcmds,char * to)
{
	int i,blen =pcmds->blen,ascii_len=0,cmd_offset=0;
	char *from=pcmds->buf;	
	struct dsi_ctrl_hdr *dchdr = (struct dsi_ctrl_hdr *)from;

	cmd_offset=sizeof(*dchdr)+dchdr->dlen ;
	...
	{	
		...
		if (i==cmd_offset)
		{
			dchdr = (struct dsi_ctrl_hdr *)(from+i);
			cmd_offset+=sizeof(*dchdr)+dchdr->dlen ;
			dchdr->dlen = ntohs(dchdr->dlen);	
			*(to+ascii_len) =0x0a;	
			*(to+ascii_len+1) =0x0d;
		...
		convert_one_byte_to_ascii(*(from+i),(to+ascii_len));			 
		 *(to+ascii_len+2) =' ';		
		 ascii_len+=3;		
	}	
...
static int save_lcd_initial_code(char * buf,int count)
{
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;
...
static int need_to_convert(char *str)
{
...
static unsigned char  ascii_to_integer(char *str,char len)
{
	
	unsigned char i,sum=0,val;
...
static int convert_initial_code_to_integer(char *from,char * to,int bytelen)
{
	int ascii_len,hex_len =0;
	char val,*ptr1,*ptr2;
	ptr1 = from;
	ptr2=to;
	...
	while(ascii_len--)
	{
		if (need_to_convert(ptr1))
		{
			if (need_to_convert(ptr1+1))
			{
				val = ascii_to_integer(ptr1,2);
				ascii_len--;
				ptr1 +=2;
			}
			else
			{
				val = ascii_to_integer(ptr1,1);
				ptr1 ++;
			}
...
static int read_lcd_initial_code(char * buffer)
{
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;		
	...
	//printk("LCD initial code test enter!!!\n");
				
	fp =filp_open("/data/lcd.txt",O_RDONLY,0644);
	if (IS_ERR(fp)){
	...
	if (!buf)
	{
		printk("LCD ERROR!!!Could not malloc initial code buffer!!!----\n");
		return -ENOMEM;
	}

	initial_code_len = vfs_read(fp,buf, MAX_INITIAL_CODE_LEN, &pos);

	initial_code_len = convert_initial_code_to_integer(buf,buffer,initial_code_len);


	printk("LCD read initial code len (hex)---------=%d\n",initial_code_len);
	
	for (i=0;i<initial_code_len;i++)
	{
		if (i%8==0)
			printk("\n");
	...
	//printk("LCD initial code test exit!!!\n");
...
static int parse_lcd_initial_code(struct dsi_panel_cmds *pcmds,char* buffer,int bytelen)
{
	int blen = bytelen,len;
	char *buf=buffer, *bp;
	...
	while (len >= sizeof(*dchdr)) {
		...
		dchdr->dlen = ntohs(dchdr->dlen);		
		if (dchdr->dlen > len) {
			pr_err("LCD parse error !!! dlen=%d",dchdr->dlen);
			goto exit_free;
		}
		bp += sizeof(*dchdr);
		len -= sizeof(*dchdr);
	...
	if (len != 0) {
		pr_err("LCD parse error !!!: dcs_cmd=%x blen=%d error!",
				buf[0], blen);
		goto exit_free;
	}

	// free first,and then malloc
	kfree(pcmds->cmds);
	...
	printk("LCD parse initial code OK!!: dcs_cmd=%x len=%d, cmd_cnt=%d link_state=%d\n", 
...
#ifdef ENABLE_LCD_DEBUG	
	 if (first_time_on_flag==1)
	 {	
	 	printk("LCD first time on,save initial code, go back!!\n");		

  		initial_code_len=convert_lcd_initial_code_to_ascii(&ctrl->on_cmds,initial_code_buf);
		save_lcd_initial_code(initial_code_buf,initial_code_len);	 	
		first_time_on_flag = 0;
		
	 }
	 else
	 {
		if (read_lcd_initial_code(initial_code_buf)==0)
		{
			parse_lcd_initial_code(&ctrl->on_cmds,initial_code_buf,initial_code_len);
		}
		else
		{
			printk("LCD read initial code error!!!\n");
		}
	 }
#endif
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	printk("tangzhengbo parse cmd111111111111111");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-command", "qcom,mdss-dsi-on-command-state");
	printk("tangzhengbo parse cmd22222222222222");
