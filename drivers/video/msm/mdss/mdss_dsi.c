...
#if defined (CONFIG_BOARD_ORION)
	mdss_dsi_panel_3v_power(pdata, 0);//pan
	printk("tangzhengbo debug convert chip rst off\n");
	mdss_convert_chip_reset(pdata, 0);
#endif
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	for (i = 0; i < DSI_MAX_PM; i++) {
		...
		//pan add
		if ((DSI_PANEL_PM == i) && (power_flag==0))
		{
			power_flag = 1;
			ret = msm_dss_enable_vreg(
			ctrl_pdata->power_data[i].vreg_config,
			ctrl_pdata->power_data[i].num_vreg, 1);
			printk("LCD enable panel powe!");
			if (ret) {
				pr_debug("LCD error!%s: failed to enable vregs for %s\n",
				__func__, __mdss_dsi_pm_name(i));
			power_flag = 0;
			}
		}
	...
	mdss_dsi_panel_5v_power(pdata, 1);//pan
	
#if defined (CONFIG_BOARD_ORION)
	mdss_dsi_panel_3v_power(pdata, 1);//pan
	printk("tangzhengbo debug convert chip rst\n");
	mdss_convert_chip_reset(pdata, 1);
#endif
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->convert_rst_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
		"zte,convert-chip-reset-gpio", 0);
	printk("tangzhengbo convert chip reset\n");
	if (!gpio_is_valid(ctrl_pdata->convert_rst_gpio)) {
		pr_err("%s:%d, qcom,convert-chip-reset-gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->convert_rst_gpio, "convert_rst_gpio");
		if (rc) {
			pr_err("request lcd_3v_vsp_en_gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->convert_rst_gpio);
			return -ENODEV;
		}
	}
