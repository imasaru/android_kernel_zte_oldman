...
static char post_sensor_module_name[SENSOR_NAME_MAX_SIZE];
...
void ov5670_read_eeprom_init(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),0x3d8a,0x70,
		MSM_CAMERA_I2C_BYTE_DATA);
	e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),0x3d8b,0x33,
		MSM_CAMERA_I2C_BYTE_DATA);
...
//add for ov5670 eeprom end by zte_cam_wxl_20150915

//add for ov5695 eeprom begin by zte_cam_wxl_20150918
void ov5695_read_eeprom_init(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	uint16_t temp;
			...
			e_ctrl->i2c_client.i2c_func_tbl->i2c_read(&(e_ctrl->i2c_client),0x3d84,&temp,
			MSM_CAMERA_I2C_BYTE_DATA);
	CDBG("lijianjun 1 %s:0x3d84 temp=0x%X \n",__func__,temp);
	...
	e_ctrl->i2c_client.i2c_func_tbl->i2c_read(&(e_ctrl->i2c_client),0x3d84,&temp,
			MSM_CAMERA_I2C_BYTE_DATA);
	CDBG("lijianjun 2 %s:0x3d84 temp=0x%X \n",__func__,temp);
...
int32_t ov5695_check_module_info_group(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	e_ctrl->i2c_client.i2c_func_tbl->i2c_read(&(e_ctrl->i2c_client),0x700C,&temp,
			MSM_CAMERA_I2C_BYTE_DATA);
	CDBG("lijianjun %s:temp=0x%X",__func__,temp);
	if((temp&0xC0) == 0x40)
	{
		return Group_One;
	}else if((temp&0x30) == 0x10)
			      ...
			      struct msm_eeprom_memory_block_t *block)
{
	...
	for (i = 0; i< block->num_map; i++) {		
		e_ctrl->i2c_client.client->addr = emap[i].mem.addr;		
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
				&(e_ctrl->i2c_client), 0,
				memptr, emap[i].mem.valid_size);
...
//add for ov5695 eeprom end by zte_cam_wxl_20150918
				       ...
				       struct msm_eeprom_memory_block_t *data)
{
	...
	for (i = 0; i < data->num_map; i++) {
		...
		rc = of_property_read_u32_array(of, property,
				(uint32_t *) &map[i].mem, count);		
		pr_err("lijianjun  addr = 0x%x, data = 0x%x\n",map[i].mem.addr,map[i].mem.data);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
			goto ERROR;
		}
...
if(strcmp(eb_info->eeprom_name,"common_t4k35")==0){
	rc = t4k35_read_eeprom_memory(e_ctrl, &e_ctrl->cal_data);
}else
if(strcmp(eb_info->eeprom_name,"sunrise_pc0fe")==0){
	rc = ov5670_read_eeprom_memory(e_ctrl, &e_ctrl->cal_data);
}else
if(strcmp(eb_info->eeprom_name,"common_ov5695")==0){
	rc = ov5695_read_eeprom_memory(e_ctrl, &e_ctrl->cal_data);
}
#endif
	if (rc < 0) {
