...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->mpp_cfg->pwm_mode == PWM_MODE) {
			...
			if (!led->mpp_cfg->pwm_cfg->blinking) {		//zte jiangfeng add //only for blink=0
			...
			{
			#ifdef CONFIG_BOARD_ACHILL //p890t09 for T-mobile
			    rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,0.5*1000000,10*1000000);//blink 0.5s per 10s .T-mobile
			#else
			    //blink //only for blink=1 ,duty cycles
			    rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,0.5*1000000,3*1000000);//blink 0.5s per 3s .baseline
			#endif
			}
			// zte jiangfeng add, end
			if (rc < 0) {
		...
		//zte jiangfeng add  //change to 10mA for orange color looks good.
		if(led_all_open() && green_led)
		{
		...
		if (led->mpp_cfg->pwm_mode != MANUAL_MODE) {
			...
			led->mpp_cfg->pwm_mode =
				led->mpp_cfg->pwm_cfg->default_mode;
			if(led_all_closed())	//zte jiangfeng add //only 1 pwm in 8610 LED for red/green
...
#if 0		//zte jiangfeng remove   //only 1 pwm so delete this part.
	if (led->mpp_cfg->pwm_mode != MANUAL_MODE)
			...
			struct pwm_config_data *pwm_cfg)
{
	...
	if (pwm_cfg->use_blink) {
		...
		pr_debug("led_blink:%s  br=%d  blink=%d\n",led->cdev.name,led->cdev.brightness,led->cdev.blink_value);	//ZTE LOG
		pwm_free(pwm_cfg->pwm_dev);
		qpnp_pwm_init(pwm_cfg, led->spmi_dev, led->cdev.name);
		led->cdev.brightness	=	0;		//zte jiangfeng add  //if adb blink test: please write blink firstly then write brightness secondly. diff with 8974/8926
#if 0
...
static int qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		//zte jiangfeng add  //change to 10mA only for green
		if(!strcmp(led->cdev.name, "green"))
		{
			green_led	=	led;
		}
		...
		if (led->default_on) {
			...
			led->cdev.brightness = LED_OFF;
			__qpnp_led_work(led, led->cdev.brightness);//led on for a long time when power on.factory request
		}
