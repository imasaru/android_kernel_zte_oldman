...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	if (pbt_buf == NULL) {
		printk("fts_ctpm_fw_upgrade_with_app_file no memory\n");
		return -1;
	}
...
int focaltech_fwupdate(struct i2c_client *client, char *fwfilename )
{
	...
	{
		printk("focaltech_fwupdate pfwfilename is null\n");
		return -1;
	}	
	if(0<=fts_read_reg(client, 0xa8,&crtFwVendorId)){
...
static unsigned char focaltech_ctpm_get_fw_ver(unsigned char* pbt_buf, unsigned int dw_lenth)
{
    if (dw_lenth > 2)
    {
...
int focaltech_get_fw_ver(struct i2c_client *client, char *fwfilename )
{
	...
	//return focaltech_ctpm_write_fw_with_app_file(client, fwfilename);
	unsigned char*     pbt_buf = 0;//focaltech_NULL;
	int ret=0xff; //u8 fwver;
	int fwsize;
	...
	if(!strcmp(fwfilename,""))
	{
		printk("focaltech_get_fw_ver pfwfilename is null\n");
		return -1;
	}	
	...
	pbt_buf = (unsigned char *) kmalloc(fwsize+1, GFP_ATOMIC);
	if (pbt_buf == NULL) {
		printk("focaltech_get_fw_ver no memory\n");
		return -1;
	}
