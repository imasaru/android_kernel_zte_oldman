...
static int fts_ts_start(struct device *dev)
{
	...
	int err;
       printk("pzh:Enter fts_ts_start\n");
	if (data->pdata->power_on) {
		err = data->pdata->power_on(true);
		if (err) {
			dev_err(dev, "power on failed");
			printk("pzh:power on failed\n");
	...
	if (data->ts_pinctrl) {
		...
		if (err < 0){
			...
			printk("pzh:Cannot get active pinctrl state\n");
		}
	}
	...
	if (err < 0) {
		...
		printk("pzh:failed to put gpios in resue state\n");
...
err_gpio_configuration:
	if (data->ts_pinctrl) {
		...
		if (err < 0){
			...
			printk("pzh:Cannot get suspend pinctrl state\n");
		}
	}
...
static void fts_ts_resume(struct work_struct *work)
{
	...
	if (!data->suspended) {
		...
		printk("pzh:Already in awake state\n");
		//return 0;
		return;
	}
	...
	int *p_fw_ver )
{
	...
	case 0x80:// 宝明
		...
		break;		
	case 0x87:// 联创
		sprintf( p_vid, "LC(0x%x)", buf1	);
		touch_moudle=LIANCHUANG;
		break;		
	case 0x8E:// 奇美
