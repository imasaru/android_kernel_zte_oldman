...
extern int syna_touch_module_for_lsensor;//xym add for lsensor
...
static struct workqueue_struct *syn_suspend_wq;
static struct work_struct	syn_suspend_work;
static struct work_struct	syn_resume_work;
...
static void synaptics_rmi4_suspend_pm(struct work_struct *work);

static void synaptics_rmi4_resume_pm(struct work_struct *work);
...
#ifdef CONFIG_BOARD_OLDMAN
void hall_state_acquire_zte(bool hall_state);
#endif
...
static int syna_ts_event(struct notifier_block *this, unsigned long event,void *ptr)
{
	int ret;
	unsigned char data;
	switch(event)
	...
	case 0:
		...
		if ( usb_plug_status != 0 ){
			...
			synaptics_rmi4_i2c_read(syn_ts,0x37,&data,1);
			data=data&0xcf;
			printk("write 0x%x to 0x37 address\n",data);
			synaptics_rmi4_i2c_write( syn_ts, 0x37,&data,1);
		}
		break;
	case 1:
		//online
...
static struct notifier_block ts_notifier = {
	.notifier_call = syna_ts_event,
};


static BLOCKING_NOTIFIER_HEAD(ts_chain_head);

int syna_register_ts_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ts_chain_head, nb);
}
EXPORT_SYMBOL_GPL(syna_register_ts_notifier);

int syna_unregister_ts_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&ts_chain_head, nb);
}
EXPORT_SYMBOL_GPL(syna_unregister_ts_notifier);
...
{
	if (syn_suspend_wq){
		queue_work(syn_suspend_wq, &syn_suspend_work);
	}
	return count;
}
	...
	int *p_fw_ver )
{
	...
	case '2':
		...
		sprintf(p_sensor, "Ec_Truly(0x%x)",ts->config_id.sensor);
	...
	syna_touch_module_for_lsensor = touch_moudle;//xym add for lsensor
...

{
	...
	if(lock==1){
		printk("ts already in update return\n");
		return 0;
	}else
	{
		lock=1;
...
#ifdef CONFIG_BOARD_ACHILL
       uint8_t success_data0[1]={0x32};
	
	if(ts->config_id.sensor==0x34)//OFILM
	{
		int retval=0;
		printk("%s: xym, change the ofilm fw config!\n", __func__); 
		
		retval=synaptics_rmi4_i2c_write(ts,0x0161,success_data0,1); //F54_ANALOG_CTRL56 
		if (retval < 0) dev_err(&(ts->input_dev->dev), "%s: Failed to write 0D Fast Relaxation Rate\n", __func__);
...
*/
	...
	syn_suspend_wq = create_singlethread_workqueue("syn_suspend_wq");
	if (!syn_suspend_wq) {
		pr_err("Could not create work queue syn_suspend_wq: no memory");
		retval = -ESRCH;
		goto err_create_singlethread;
	}
	INIT_WORK(&syn_suspend_work, synaptics_rmi4_suspend_pm);
	INIT_WORK(&syn_resume_work, synaptics_rmi4_resume_pm);
...
#if 1//defined(CONFIG_TOUCHSCREEN_FOCALTECH_USBNOTIFY)
			  syna_register_ts_notifier(&ts_notifier);
#endif
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_UNBLANK)
			queue_work(syn_suspend_wq, &syn_resume_work);
...
void hall_state_acquire_zte(bool hall_state)
{
	if(syn_suspend_wq && syn_ts){
		if(!hall_state){
			if (!syn_ts->sensor_sleep) {
				printk("%s:synaptics-ts suspend caused by hall\n" , __func__);
				queue_work(syn_suspend_wq, &syn_suspend_work);
			}
		}else {
			if (syn_ts->sensor_sleep) {
				printk("%s:synaptics-ts resume caused by hall\n" , __func__);
				queue_work(syn_suspend_wq, &syn_resume_work);
			}
		}
	}
...
static void synaptics_rmi4_resume_pm(struct work_struct *work)
{
	...
	if(syn_ts->sensor_sleep){
		printk("%s:sensor has slept!\n", __func__);
		synpatics_rmi4_pinctrl_select(syn_ts,true);
