		...
		struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  do {			
    ...
    prox_mean = 0;
    count_flag ++;
    printk("%s: poll---> prox_pulse = %d, count = %d\n", __FUNCTION__, prox_pulse, count_flag);
    for (i = 0; i < 20; i++) 
    {
    ...
    printk("%s: xym init prox_pulse(%d), pluse(%d), sat_prox(%d), hi(%d), lo(%d), prox_mean(%d), g(0x%0x)\n", __FUNCTION__,
    ...
    /*printk("%s: prox_pulse_cnt_param=%d, pluse(%d),sat_prox(%d), hi(%d), lo(%d), c(%d), g(0x%0x)\n", __FUNCTION__,
        prox_pulse_cnt_param,
        prox_pulse,
        sat_prox,
        taos_cfgp->prox_threshold_hi,
