...
#if defined(CONFIG_BOARD_ACHILL)
...
static void report_als_input_event(struct ltr559_data *ltr559)
{
	...
	//LTR559_DBG("%s: adc_value_raw=%d\n", __func__, adc_value);
	LTR559_DBG2("%s: adc_value=%d\n", __func__, adc_value);
...
static int8_t ps_enable_init(struct ltr559_data *ltr559)
{
	...
	rc = ps_ledPulseCount_setup(4, ltr559);//p pulse 4
	if (rc < 0) {
		dev_err(&ltr559->i2c_client->dev,
...
#if defined(CONFIG_BOARD_ABBY)//P890A57
    if(prox_mean < 1200){
...
#else
    if(prox_mean < 1200){
        prox_threshold_hi_param = prox_mean + 70; //40;
		prox_threshold_lo_param = prox_mean + 35; //22;
    } else {
        prox_threshold_hi_param = 1900;
		prox_threshold_lo_param = 1500;
    }
