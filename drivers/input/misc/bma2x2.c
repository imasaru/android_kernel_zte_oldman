...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		...
		if (pre_enable == 1) {
			...
			bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND);
			//bma2x2_pinctrl_state(bma2x2, false);//xym deleted
			if ((bma2x2->pdata->int_en) &&
				(BMA2x2_IS_NEWDATA_INT_ENABLED())) {
...
static int bma2x2_pinctrl_init(struct bma2x2_data *data)
{
	...
	//pctrl_data->pins_sleep = pinctrl_lookup_state(pctrl, "sleep");//xym deleted
		...
		const struct i2c_device_id *id)
{
	...
	//printk("%s: cgh please notify 0x%X\n", __func__, client->addr);
	RESET_DELAY();
	if (bma2x2_soft_reset(client) < 0) {
		//data->bma2x2_client->addr = 0x18;
	...
	//bma2x2_pinctrl_state(data, false);//xym deleted
