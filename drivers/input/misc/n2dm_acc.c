...
static int n2dm_acc_enable(struct n2dm_acc_status *stat)
{
	...
	//pr_info("%s: stat->enabled=%d\n", __func__, atomic_read(&stat->enabled));//xym add 20151207
...
static struct device_attribute attributes[] = {
	...
	//__ATTR(enable, 0664, attr_get_enable, attr_set_enable),//xym temp 20151207
...
static void n2dm_acc_input_work_func(struct work_struct *work)
{
	...
	//pr_info("%s: stat->enabled=%d\n", __func__, atomic_read(&stat->enabled));//xym add 20151207
	if(atomic_read(&stat->enabled)==0) return;//xym add 20151207
...
int n2dm_acc_input_open(struct input_dev *input)
{
	...
	pr_info("%s: \n", __func__);
	//return n2dm_acc_enable(stat);//xym 20151207
	return 0;
}
...
void n2dm_acc_input_close(struct input_dev *dev)
{
	...
	pr_info("%s: \n", __func__);
	//n2dm_acc_disable(stat);//xym 20151207
