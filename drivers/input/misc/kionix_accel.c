...
static void kionix_accel_update_direction(struct kionix_accel_driver *acceld)
{
	...
	case KIONIX_ACCEL_GRP5:
		acceld->axis_map_x = (direction % 2);//P890A13  direction=7  1,0,2
		acceld->axis_map_y = ((direction - 1) % 2);
		acceld->negate_x = (((direction + 1) / 2) % 2);//1
		acceld->negate_y =
		    (((direction / 2) + ((direction - 1) / 4)) % 2);
		pr_info("%s: direction=%d,  axis_map_xyz=%d,%d,%d, negate_xyz=%d,%d,%d\n", 
			__func__, direction, 
			acceld->axis_map_x, acceld->axis_map_y, acceld->axis_map_z, 
			acceld->negate_x, acceld->negate_y, acceld->negate_z );
		break;
	default:
...
{
#ifdef KIONIX_VDD_HOLD_FOR_BOARD_TEST
	...
	if (!on) {
		...
		if (regulator_count_voltages(vdd_hold) > 0)
			regulator_set_voltage(vdd_hold, 0, KIONIX_VDD_MAX_UV);
		...
		if (regulator_count_voltages(vdd_hold) > 0) {
			rc = regulator_set_voltage(vdd_hold, KIONIX_VDD_MIN_UV,
						   KIONIX_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->client->dev,
