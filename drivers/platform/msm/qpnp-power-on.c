...
struct qpnp_pon {
	...
	struct work_struct pwrkey_poweroff_work;	//zte jiangfeng added
...
static void check_pwrkey(struct work_struct *work)
{
	...
	{
		del_timer(&pon->timer);
		printk("%s, power key not pressed, delete timer of power key\n", __func__);
	}
}
//zte jiangfeng add, end
