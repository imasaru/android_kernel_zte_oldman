...
//yeganlin_20150113, add for blkdev_ioctl to get the write protected status
...
void set_partition_ro(struct mmc_card *card, struct mmc_blk_data *md, const char *partition,int mergerd_num)
{
	...
	for(/*i = 1*/; i < MAX_PARTION_NUMBER; i++){
	    //pr_err("ygl set_partition_ro()::i=%d\n",i);
	    ...
	    if (p) {
	        if (p->info && p->info->volname[0]){
	            ...
	            pr_err("ygl set_partition_ro()::e,wp_status_buf[7]=0x%x\n",wp_status_buf[7]);
	            if((wp_status_buf[7] & 0x3) == POWER_ON_WP_STATUS){ //"10"
	                //pr_err("ygl set_partition_ro()::set partition to ro,partno=%d\n",p->partno);
	                p ->policy = 1;
	                md->writeprotected_status = 1; //yeganlin_20150113, add for blkdev_ioctl to get the write protected status
	            }else{
	                disk_put_part(p);
	                break;
	            }
	            ...
	            for(j = 1;j< mergerd_num; j++){
	                ...
	                if (p_next) {
	                    //pr_err("ygl set_partition_ro()::partno=%d,j=%d\n",p_next->partno,j);
	                    p_next ->policy = 1;
	                }
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if(mmc_card_mmc(card)){
		...
		if (strstr(saved_command_line, RECOVERY_MODE_STR)) {
			...
			set_partition_ro(card,md,"sbl1",16); /*sbl1+DDR+rpm+tz+fsg+sec+devinfo+echarge+splash+aboot+modem+boot+recovery+pad+pad1+ztelk */
