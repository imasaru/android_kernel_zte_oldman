...
{
    /**ZTE_MODIFY LiuTao 2015-12-24, fake button press while inserting headset */    
    if((mbhc->reject_btn) && (jack == &mbhc->button_jack)) {
        pr_err(" liut: %s: do not jack_report\n", __func__);
        return;
    }
    /*ZTE_MODIFY_END*/
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
        /**ZTE_MODIFY LiuTao 2015-12-24, fake button press while inserting headset */
        if(mbhc->reject_btn) {
            pr_debug("liut %s: headset removed, set mbhc->reject_btn false;\n", __func__);
        ...
        if(jack_type & SND_JACK_HEADSET) {
			pr_debug("liut %s: set reject_btn true;\n", __func__);
    		mbhc->reject_btn = true;
    		dev_info(codec->dev, "%s: liut delay btn det %d\n", __func__, mbhc->reject_btn);
    		cancel_delayed_work(&mbhc->btn_work);
    		queue_delayed_work(mbhc->btn_workqueue, &mbhc->btn_work, msecs_to_jiffies(2500));
        }
        /*ZTE_MODIFY_END*/
		/*
		...
		if (jack_type == SND_JACK_HEADPHONE)
			...
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_GND_MIC_SWAP;	// ZTE_MODIFY LiuTao 20150723, for pv & fastmmi
		else if (jack_type == SND_JACK_HEADSET) {
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_HEADSET;		// ZTE_MODIFY LiuTao 20150723, for pv & fastmmi
			mbhc->jiffies_atreport = jiffies;
		}
		else if (jack_type == SND_JACK_LINEOUT)
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_HIGH_HPH;		// ZTE_MODIFY LiuTao 20150723, for pv & fastmmi
...
static void wcd_mbhc_reject_btn_fn(struct work_struct *work)
{
       ...
       pr_err(" liut:release btn det reject_btn(%d) \n", mbhc->reject_btn);
}
/*ZTE_MODIFY_END*/
		      ...
		      bool impedance_det_en)
{
    ...
    if (mbhc->btn_workqueue == NULL) {
        pr_err(" liut: can't create work queue \n");
    }
    INIT_DELAYED_WORK(&mbhc->btn_work, wcd_mbhc_reject_btn_fn);
    pr_debug("liut %s: init wcd_mbhc_reject_btn_fn fn\n", __func__);
    /*ZTE_MODIFY_END*/
