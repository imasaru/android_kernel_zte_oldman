...
/*
                ...
                delta = (unsigned long)(now - curr->statistics.last_cpu_busy_mark);

                if ( delta < 250000000 && entity_is_task(curr) &&
                        strncmp(curtask->comm, "top", 3) && strncmp(curtask->comm, "procrank", 8)){
                    if (curtask->pid != curtask->group_leader->pid)
                        trace_printk("cpu-usage: %d[%s]-%d[%s] runs %lld ms\n", curtask->pid, curtask->comm,
                                curtask->group_leader->pid, curtask->group_leader->comm, nsec_high(curr->sum_exec_runtime));
                    else
                        trace_printk("cpu-usage: %d[%s] runs %lld ms\n", curtask->pid, curtask->comm, nsec_high(curr->sum_exec_runtime));
                }
*/
